...
if [ $target_res = 0.00003086420 ]; then
	...
	gmt grdconvert $grid_dem "tifs/"$basename_"_DEM.tif"=gd:GTiff
	...
	#smooth 1_3 version if not running in arcpy mosaic
	smooth_dem_bathy.py "tifs/"$basename_"_DEM_tmp.tif" -s $smooth_factor
	mv "tifs/"$basename_"_DEM_tmp_smooth_"$smooth_factor".tif" "tifs/smoothed/1_3/"$basename_"_DEM_smooth_"$smooth_factor".tif"
